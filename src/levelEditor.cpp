...
void LevelEditor::updateProjectionMatrix(int width, int height) {
...
void LevelEditor::framebuffer_size_callback(GLFWwindow* window, int width, int height) {
    glViewport(0, 0, width, height);

    // Retrieve the Game instance from the window user pointer
    LevelEditor* levelEditor = static_cast<LevelEditor*>(glfwGetWindowUserPointer(window));

    // Update the projection matrix
    levelEditor->updateProjectionMatrix(width, height);
}
