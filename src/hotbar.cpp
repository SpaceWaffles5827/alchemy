...
HotBar::HotBar()
    : Renderable(183.0f * 3, 23.0f * 3, textureID, glm::vec2(0.0f, 1.0f),
                 glm::vec2(1.0f, 0.0f),
                 glm::vec3(400.0f, 800.0f - 35.0f, 0.0f)),
      width(1.0f), height(1.0f), textureID(0),
      texTopLeft(glm::vec2(0.0f, 0.0f)), texBottomRight(glm::vec2(1.0f, 0.0f)),
      selectedSlotIndex(0) {
  initializeSlots();
  GLuint selectedSlotTexture =
      GraphicsContext::getInstance().loadTexture("textures/ui/selected.png");
  selectedSlotObject = Renderable(23.0f * 3, 23.0f * 3, selectedSlotTexture,
                                  glm::vec2(0.0f, 1.0f), glm::vec2(1.0f, 0.0f),
                                  glm::vec3(400.0f - 239.0f, 765.0f, 0.0f));
}
...
Renderable HotBar::getSelectedSlotObject() { return selectedSlotObject; }
...
void HotBar::loadDefaults() {
  GLuint specialTextureID = GraphicsContext::getInstance().loadTexture(
      "textures/world/stone_bricks.png");
  for (int i = 0; i < slots.size(); ++i) {
...
std::vector<InventorySlot> &HotBar::getHotBarSlots() { return slots; }
...
void HotBar::setTexture(GLuint newTextureID, const glm::vec2 &newTexTopLeft,
                        const glm::vec2 &newTexBottomRight) {
  textureID = newTextureID;
  texTopLeft = newTexTopLeft;
  texBottomRight = newTexBottomRight;

  Renderable::setTexture(newTextureID);
  Renderable::setTextureCoords(
      glm::vec2(texTopLeft.x, 1.0f - texTopLeft.y),
      glm::vec2(texBottomRight.x, 1.0f - texBottomRight.y));
}
