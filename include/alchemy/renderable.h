...
class Renderable {
  public:
    ...
    void setTextureCoords(const glm::vec2 &topLeft,
                          const glm::vec2 &bottomRight);
    void setYSortOffset(float offset);

    void updateScale();
    void updateBoundingRadius();

    float getBoundingRadius() const;
    bool getIsYSorted() const;
    void setYSorting(bool status);

    // New method to calculate base Y position
    float calculateBaseYPosition() const;
    glm::vec3 getYSortPosition() const;
  ...
  protected:
    ...
    bool isVisable;
    bool ySort;
    float ySortOffset; // New member variable for Y-sort offset
};
